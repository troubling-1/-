    ...
    <div className="mx-auto max-w-7xl px-4 py-10">
      <div>
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          当前页面已启用服务端权限校验。只有 role=admin 且 status=active 的账号才能看到后台内容。
        </p>
      </div>
      ...
      <Card className="mt-6 border-amber-300/20 bg-amber-300/10">
        <CardContent className="p-5 text-sm leading-6 text-amber-100">
          数据来源说明：护航师审核读取 Supabase 数据库 escort_applications；通过/拒绝操作走管理员 API。
          当前这个后台页面没有展示订单管理、提现管理、举报管理模块；这些模块如果后续恢复，必须同样接入服务端管理员校验。
        </CardContent>
      </Card>
            ...
            <div key={application.id} className="grid gap-4 rounded-md border border-border bg-black/30 p-4 lg:grid-cols-[1fr_320px]">
              ...
              <div className="grid gap-3">
                ...
                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1">
                  <Button type="button" variant="outline" disabled={application.status !== "pending"} onClick={() => handleReview(application.id, "approve")}>
                    通过申请
                  </Button>
                  <Button type="button" variant="danger" disabled={application.status !== "pending"} onClick={() => handleReview(application.id, "reject")}>
                    拒绝申请
                  </Button>
                </div>
              </div>
